...
func IsActive(h *DisplayHandler) bool {
	if err := h.ioctl(I2C_SLAVE, EDID_ADDR); err != nil {
		return false
	}
...
// GetEDID will try to read the EDID package at address 
// 0x50, for now we only support he decoding of descriptor
// and header block. 
// 
...
func GetEDID[E EDIDBlock](h *DisplayHandler) (E, error) {
...
func getEDIDChecksum(buf []byte) byte {
	var out byte
	for i := 0; i < 128; i++ {
		out += buf[i]
	}
